...
@section('title', 'Edit Grade Teacher Assignment')
...
<div class="page-content">
    ...
    <div class="card">
        <div class="card-body p-4">
            <h5 class="card-title">Edit Grade Teacher Assignment</h5>
            <hr />
            <form action="{{ route('admin.grade_teachers.update', $gradeTeacher->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-lg-4 mb-3">
                        ...
                        <select name="grade_id" id="grade_id" class="form-select @error('grade_id') is-invalid @enderror" required>
                            ...
                            @foreach ($grades as $grade)
                                <option value="{{ $grade->id }}" {{ old('grade_id', $gradeTeacher->grade_id) == $grade->id ? 'selected' : '' }}>
                                    {{ $grade->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-4 mb-3">
                        ...
                        <select name="teacher_id" id="teacher_id" class="form-select @error('teacher_id') is-invalid @enderror" required>
                            ...
                            @foreach ($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('teacher_id', $gradeTeacher->teacher_id) == $teacher->id ? 'selected' : '' }}>
                                    {{ $teacher->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-lg-4 mb-3">
                        ...
                        <select name="subject_id" id="subject_id" class="form-select @error('subject_id') is-invalid @enderror" required>
                            ...
                            @foreach ($subjects as $subject)
                                <option value="{{ $subject->id }}" {{ old('subject_id', $gradeTeacher->subject_id) == $subject->id ? 'selected' : '' }}>
                                    {{ $subject->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="border border-3 p-4 rounded mt-3">
                    <button type="submit" class="btn btn-primary radius-30" style="background-color: #244960">Update Assignment</button>
                </div>
            </form>
        </div>
    </div>
