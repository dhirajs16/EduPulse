                ...
                <div class="form-body mt-4">
                    <div class="row">
                        ...
                        <div class="col-lg-6 mb-3">
                            ...
                            <select name="grade_id" id="grade_id" class="form-select @error('grade_id') is-invalid @enderror" required>
                                ...
                                @foreach ($grades as $grade)
                                    <option value="{{ $grade->id }}" {{ old('grade_id', $fee->grade_id) == $grade->id ? 'selected' : '' }}>
                                        {{ $grade->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg-4 mb-3">
                            ...
                            <input type="number" step="0.01" min="0" name="amount" id="amount" class="form-control @error('amount') is-invalid @enderror" value="{{ old('amount', $fee->amount) }}" required>
                            @error('amount')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-lg-4 mb-3">
                            ...
                            <input type="number" min="1900" max="{{ date('Y') + 5 }}" name="year" id="year" class="form-control @error('year') is-invalid @enderror" value="{{ old('year', $fee->year) }}" required>
                            @error('year')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-lg-4 mb-3">
                            ...
                            <input type="number" min="1" max="12" name="month" id="month" class="form-control @error('month') is-invalid @enderror" value="{{ old('month', $fee->month) }}" required>
                            @error('month')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="border border-3 p-4 rounded">
                        <button type="submit" class="btn btn-primary radius-30" style="background-color: #244960">Update Fee</button>
                    </div>
                </div>
            </form>
        </div>
