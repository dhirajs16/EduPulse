...
<div class="page-content">
    ...
    <div class="card">
        <div class="card-body p-4">
            ...
            <h5 class="card-title mb-4">Edit Syllabus</h5>

            <form method="POST" action="{{ route('admin.syllabi.update', $syllabus->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select id="grade_id" name="grade_id" class="form-select @error('grade_id') is-invalid @enderror" required>
                        ...
                        @foreach($grades as $grade)
                            <option value="{{ $grade->id }}" {{ old('grade_id', $syllabus->grade_id) == $grade->id ? 'selected' : '' }}>
                                {{ $grade->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" id="chapter_number" name="chapter_number" min="1" class="form-control @error('chapter_number') is-invalid @enderror" value="{{ old('chapter_number', $syllabus->chapter_number) }}" required>
                    @error('chapter_number')<span class="text-danger">{{ $message }}</span>@enderror
                </div>

                <div class="mb-3">
                    <label for="title" class="form-label">Title <span class="text-danger">*</span></label>
                    <input id="title" name="title" type="text" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $syllabus->title) }}" required>
                    @error('title')<span class="text-danger">{{ $message }}</span>@enderror
                </div>

                <div class="mb-3">
                    <label for="sub_topics" class="form-label">Sub Topics <span class="text-danger">*</span></label>
                    <textarea id="sub_topics" name="sub_topics" rows="4" class="form-control @error('sub_topics') is-invalid @enderror" required>{{ old('sub_topics', $syllabus->sub_topics) }}</textarea>
                ...
                <div class="mb-3">
                    ...
                    <input id="credit_hours" name="credit_hours" type="number" min="1" class="form-control @error('credit_hours') is-invalid @enderror" value="{{ old('credit_hours', $syllabus->credit_hours) }}" required>
