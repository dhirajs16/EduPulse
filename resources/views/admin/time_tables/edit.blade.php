...
<div class="page-content">
        ...
        <div class="ps-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Edit Timetable Entry</li>
                </ol>
            </nav>
        </div>
        <div class="ms-auto">
    ...
    <div class="card">
        <div class="card-body p-4">
            <h5 class="card-title">Edit Timetable Entry</h5>
            <hr />
            <form action="{{ route('admin.time-tables.update', $timeTable->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-body mt-4">
                    <div class="row">
                        ...
                        <div class="col-lg-3 mb-3">
                            ...
                            <input type="time" name="start_time" id="start_time" class="form-control @error('start_time') is-invalid @enderror" value="{{ old('start_time', $timeTable->start_time) }}" required>
                            @error('start_time')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-lg-3 mb-3">
                            ...
                            <input type="time" name="end_time" id="end_time" class="form-control @error('end_time') is-invalid @enderror" value="{{ old('end_time', $timeTable->end_time) }}" required>
                            @error('end_time')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-lg-3 mb-3">
                            ...
                            <select name="grade_id" id="grade_id" class="form-select @error('grade_id') is-invalid @enderror" required>
                                ...
                                @foreach ($grades as $grade)
                                    <option value="{{ $grade->id }}" {{ old('grade_id', $timeTable->grade_id) == $grade->id ? 'selected' : '' }}>
                                        {{ $grade->name }}
                                    </option>
                                @endforeach
                            </select>
