        ...
        <div class="card-body p-4">
            ...
            <form method="POST" action="{{ route('admin.books.update', $book->id) }}" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    @if($book->cover_image)
                        <img src="{{ asset('storage/'.$book->cover_image) }}" alt="Cover Image" height="80" class="mt-2 rounded" />
                    @endif
                    @error('cover_image')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="publication_year" id="publication_year" min="0" max="{{ date('Y') }}"
                        class="form-control @error('publication_year') is-invalid @enderror" value="{{ old('publication_year', $book->publication_year) }}">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="category_name" id="category_name" class="form-control @error('category_name') is-invalid @enderror"
                        value="{{ old('category_name', $book->category_name) }}">
                    @error('category_name')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
